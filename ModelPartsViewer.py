...
def viewNextModel(viewer):
    modelPartsViewer.viewerModelIndex = (modelPartsViewer.viewerModelIndex+1) % len(modelPartsViewer.models)
...
def viewNextPart(viewer):
    ...
    modelPartsViewer.renderMode = (modelPartsViewer.renderMode + 1) % (totalPartsCount + 1)
...
def start():
    ...
    pyrender.Viewer(defaultScene, registered_keys = {'d' : viewNextModel, 'a':viewPrevModel, 's':viewPrevPart, 'w':viewNextPart}, use_raymond_lighting=True)
